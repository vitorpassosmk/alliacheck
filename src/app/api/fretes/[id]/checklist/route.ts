...
export async function POST(
  ...
  if (marcado) {
    ...
    if (upsertError) return Response.json({ error: 'Erro ao marcar item' }, { status: 500 })
  } else {
    const { error: deleteError } = await supabase
      .from('checklist_respostas').delete().eq('frete_id', id).eq('item_id', item_id)
    if (deleteError) return Response.json({ error: 'Erro ao desmarcar item' }, { status: 500 })
  }
